...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		// cancel button (jump to next field)
		this.ctrlCancel = createElement( 'button', { cName : 'fs-cancel side', inner : 'Cancel', appendTo : this.ctrls } );
		this._showCtrl( this.ctrlCancel );
	...
	FForm.prototype._initEvents = function() {
		...
		// keyboard navigation events - jump to next field when pressing enter or cancel if escape
		...
		var self = this,
			onEndAnimationFn = function( ev ) {
				...
				if( self.isLastStep ) {
					...
					self._hideCtrl( self.ctrlCancel );
...
$( document ).ready(function() {
	...
	// check sass line 12 _variabless.scss
	if (screen.width <= 400) {
		$(".slide:first-child .next-slide").click(function () {
			$(this).closest(".slide").siblings().addClass("active");
		});
	} else {
		...
		if ($("body").hasClass("section-contact-us") || $("body").hasClass("node-type-article")) {
			/* Menu Navigation effects */
			...
			if ($("body").hasClass("node-type-article")) {
				// Active element for article
				$(".menu-2405 a").addClass('active');
			}

		} else {
			$("nav#main-nav").headroom({
			  "offset": $("nav#main-nav").offset().top+50,
			  "tolerance": 20,
			  "classes": {
			  	"initial": "initial",
			    "pinned": "pinned",
		...
		/* Center the screen on navigation first button */
		$(".slide:first-child .next-slide, .slide-control").click(function () {

			var parent = $(this).closest(".slide");
			var target_position = Math.floor(parent.offset().top + (parent.height()/2 - $(window).height()/2));

			if ($(window).scrollTop() != target_position) { 
				$('html, body').animate({
					scrollTop: target_position
				}, 300, function() {
					// Animation complete.
					//fix menu pinned to not show
					$("nav#main-nav").addClass("slideUp");				
				});
			}
			$("nav#main-nav").addClass("slideUp");	
		...
		/* Next slide when press button */
		$(".next-slide").click(function () {
			if ($(this).closest(".slide").next().length) {
				$(this).closest(".slide").removeClass("active").next().addClass("active");
			}
			
		});

		/* Previous slide when press button */
		$(".prev-slide").click(function () {
			if ($(this).closest(".slide").prev().length) {
				$(this).closest(".slide").removeClass("active").prev().addClass("active");
			}
		});

		/* Go to first slide when press button */
		$(".close-slide").click(function () {
				$(this).closest(".slide").removeClass("active").siblings().first().addClass("active");
		});
	...
	/* Close Booking Form */
	$("button.fs-cancel").click(function () {
		$("body").removeClass("contact-form-active");
	});	
	...
	//Close message box
	$("body").click(function () {
		$(".messages").addClass("messages-close");
	});	


	// class to btns in journal
	$("body.section-journal .field-group-format-wrapper a").addClass('btn')
